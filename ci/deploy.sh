...
case $OSTYPE in
  darwin*)
    zipdir=Kaguya-macos-$TAG_NAME
    if [[ "$BUILD_REASON" == "Schedule" ]] ; then
      zipname=Kaguya-macos-nightly.zip
    else
      zipname=$zipdir.zip
    fi
    ...
    mkdir -p $zipdir/Kaguya.app/Contents/MacOS
    mkdir -p $zipdir/Kaguya.app/Contents/Resources
    cp -r assets/shell-integration/* $zipdir/Kaguya.app/Contents/Resources
    cp -r assets/shell-completion $zipdir/Kaguya.app/Contents/Resources
    tic -xe wezterm -o $zipdir/Kaguya.app/Contents/Resources/terminfo termwiz/data/wezterm.terminfo

    for bin in kaguya wezterm-mux-server kaguya-gui strip-ansi-escapes ; do
      ...
      if [[ -f $TARGET_DIR/release/$bin ]] ; then
        cp $TARGET_DIR/release/$bin $zipdir/Kaguya.app/Contents/MacOS/$bin
      else
        ...
        lipo $TARGET_DIR/*/release/$bin -output $zipdir/Kaguya.app/Contents/MacOS/$bin -create
      fi
    done
    ...
    if [ -n "$MACOS_TEAM_ID" ] ; then
      ...
      /usr/bin/codesign --keychain build.keychain --force --options runtime \
        --entitlements ci/macos-entitlement.plist --deep --sign "$MACOS_TEAM_ID" $zipdir/Kaguya.app/
